...
def compare_relevance_labels(data, queries, relevance_labels):
    ...
    combined_judgements = {
        q: sum(
            get_label_judgements(data, [q], label)[q]
            for label in relevance_labels
            if label != irrelevant_label
        )
        for q in queries
...
def analyze_query_judgements(data):
    ...
    # Add new analysis
    analysis_results["label_comparison"] = compare_relevance_labels(
        data, queries, relevance_labels
    )
...
def find_multi_query_docs(qrels):
    ...
    relevance_info = qrels.groupby("doc_id").apply(
...
def display_further_details_multi_query_docs(multi_query_docs, num_docs):
    ...
    st.write(
        f"Average number of relevance judgements across queries per document: <span style='color:red;'>{avg_queries:.2f}</span>",
        unsafe_allow_html=True,
    )
    st.write(
        f"Maximum number of relevance judgements for a single document: <span style='color:red;'>{max_queries}</span>",
        unsafe_allow_html=True,
    )

    st.write(
        f"Note that if a document has relevance judgements equal to the total number of queries in the collection it means that the document is somehow related to all the queries in the "
    ...
    st.markdown(
        f"##### Top {num_docs} Documents ranked by their relevance assessments across queries"
    )
    ...
    for _, row in top_docs.iterrows():
        ...
        relevance_counts = (
            pd.Series(row["relevance_judgments"].values()).value_counts().to_dict()
        )

        doc_info = {"Document ID": doc_id, "Rel.Judgments across queries": query_counts}
...
def find_ranked_pos_of_multi_query_docs(multi_query_docs, num_docs, experiments, qrels):
    ...
    doc_ids = docs_multi_asments["doc_id"].unique()[
        :50
    ]  # Limit to a maximum of 20 documents
    ...
    for run, run_data in experiments.items():
        ...
        merged_data = pd.merge(
            filtered_run_data, qrels, on=["query_id", "doc_id"], how="left"
        )
        ...
        for doc_id in doc_ids:
            ...
            if not doc_data.empty:
                ...
                combined_data = doc_data[
                    ["query_id", "rank", "score", "relevance_label"]
                ]

                # Display the table in the current column
                with columns[column_index]:
                    st.write(
                        f"""<center>Per query ranking details of Document: <span style="color:red;">{doc_id}</span></center>""",
                        unsafe_allow_html=True,
                    )
        ...
        if missing_docs:
            st.write(
                f"**The following documents have not been retrieved by the evaluated experiment:** {', '.join(missing_docs)}"
            )
        else:
            st.write(
                f"**All examined documents have been retrieved by the evaluated experiment.**"
            )
...
def documents_retrieved_by_experiments(dataframes_dict):
    ...
    result = grouped.agg(
        {"run": lambda x: ",".join(sorted(set(x))), "query_id": "count"}
