...
from utils.plots import (create_relevance_wordclouds,
                         plot_performance_similarity,
                         plot_query_performance_vs_query_length,
...
# Title for the page
st.markdown(
    """<div style="text-align: center;"><h1>Query Text-based Analysis Across Multiple Experiments<h1></div>""",
...
if st.button("Begin the Experimental Evaluation!", key="qmet_stButtonCenter"):
    ...
    # Load Qrels data
    st.session_state.qmet_selected_qrels = load_qrel_data(
        os.path.join(qrels_dir, qrels_file)
    )
    st.session_state.qmet_max_relevance = st.session_state.qmet_selected_qrels[
        "relevance"
    ].max()
    ...
    for run_file in selected_runs_files:
        st.session_state.qmet_selected_runs[run_file] = load_run_data(
    ...
    if queries_file:
        st.session_state.qmet_selected_queries = load_query_data(
...
if ("qmet_selected_queries" in st.session_state and not st.session_state.qmet_selected_queries.empty):
    if len(st.session_state.qmet_selected_queries) > 500:
        with st.container():
            ...
            st.write(
                f"""<div style="text-align: center;">  ⚠️ Note: Too many available queries (<span style="color:red;">{len(st.session_state.qmet_selected_queries)}</span>).
            ...
            # Main content logic
            st.write(
                f"""<div style="text-align: center;"> A total of <span style="color:red;">{st.session_state.qmet_random_size}</span> random queries have been
            selected based on a random state equal to <span style="color:red;">{st.session_state.qmet_random_state}</span> and will be used for the upcoming analyses.</div>""",
                unsafe_allow_html=True,
            )

            st.session_state.qmet_selected_queries_random = (
                st.session_state.qmet_selected_queries.sample(
                    n=st.session_state.qmet_random_size,
                    random_state=st.session_state.qmet_random_state,
                )
            )

            st.write(
                f"""<div style="text-align: center;"> Number of randomly selected queries that would be used for analysis: <span style="color:red;"
                >{len(st.session_state.qmet_selected_queries_random)}</span></div>""",
                unsafe_allow_html=True,
            )

            query_ids = np.sort(
                st.session_state.qmet_selected_queries_random.query_id.values
            )
...
with st.container():
    ...
    if "qmet_selected_runs" not in st.session_state:
        ...
        #   }
        # }
        st.session_state.qmet_query_rel_judg = plot_query_relevance_judgements(
        ...
        for i, label in enumerate(relevance_labels):
            with columns[i]:
                ...
                st.markdown(
                    f"<h5><span style='color:{color};'>{label_name}</span></h4>",
                    unsafe_allow_html=True,
                )
...
with st.container():
    st.markdown(
        """<h3>Retrieval Performance - <span style="color:red;">Query Text Analysis based on their Relevance Judgements</span></h3>""",
        unsafe_allow_html=True,
    )
    ...
    with st.expander("See Analysis Details and Interpretations"):
        st.subheader("Overview")
        st.write(
            "This section creates word-clouds based on the queries and their relevance judgements."
        )
        cola, colb = st.columns(2)
        with cola:
            st.subheader("How it works")
            st.write(
                "The analysis leverages several techniques to classify the queries based on their relevance judgments and then creates word-clouds. (See below)"
            )
        with colb:
            st.subheader("How to use")
            st.write(
                "The analysis can be useful in understanding topics in the queries, for which a few or many relevant documents exist in the collection."
            )
    ...
    if "qmet_selected_runs" not in st.session_state:
        ...
        with col2:
            ...
            if st.session_state.qmet_method_text_plots not in [
                "Percentile-based method",
                "Max-Min-Median",
            ]:
                st.session_state.qmet_threshold_text_plots = st.slider(
                    "Select threshold", 1.0, 10.0, 1.5, 0.1
                )
...
with st.container():
    st.markdown(
        """<h3>Retrieval Performance - <span style="color:red;">Query Performance based on  Query Length</span></h3>""",
        unsafe_allow_html=True,
    )
    _, _, custom_user, default_measures, _, _ = return_available_measures()
    ...
    if "qmet_selected_runs" not in st.session_state:
        ...
        if len(st.session_state.qmet_selected_measures) >= 1:
            st.session_state.per_query_length_res = per_query_length_evaluation(
                st.session_state.qmet_selected_qrels,
                st.session_state.qmet_selected_runs,
                st.session_state.qmet_selected_queries_random,
            ...
            plot_query_performance_vs_query_length(
                st.session_state.per_query_length_res
            )
...
with st.container():
    st.markdown(
        """<h3>Retrieval Performance - <span style="color:red;">Query Performance vs Query Text Similarity</span></h3>""",
        unsafe_allow_html=True,
    )
    ...
    with st.expander("See Analysis Details and Interpretations"):
        ...
        with colb:
            st.subheader("How to use")
            st.write(
                "The analysis can be used to answer questions like: Do similar queries have similar performance?"
            )
