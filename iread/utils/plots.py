...
def plot_queries(scores, ids, median, average, graph_title):
    ...
    fig = px.scatter(df, x='Query ID', y='Scores', color=df['Scores'] > median, labels={'color': 'Above Median'})
    ...
    above_median_annotation = {
        ...
        'y': max(scores) + .1,  # Adjust the vertical position to the top
    ...
    below_median_annotation = {
        ...
        'y': min(scores) - .2,  # Adjust the vertical position to the top
...
def plot_pca(pca_df, classify_queries):
    ...
    if classify_queries == 'Yes':
        ...
        fig = px.scatter(pca_df, x='PCA1', y='PCA2',
                         ...
                         color_discrete_map={'Hard Query': ' soft red', 'Easy Query': 'blue'},  # Define color mapping
        ...
        fig.update_traces(marker=dict(size=7),
                          hovertemplate='<b>Query ID: %{customdata[0]}</b>')  # Display query ID in bold

        # Update plot layout
        fig.update_layout(xaxis_title='PCA1', yaxis_title='PCA2',
                          hoverlabel=dict(font_size=16))  # Increase hover label font size
