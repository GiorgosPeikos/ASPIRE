...
from utils.eval_per_query import (analyze_performance_difference,
                                  analyze_performance_difference_median,
                                  analyze_performance_difference_threshold,
                                  analyze_performance_perq,
...
# Title for the page
st.markdown(
    """<div style="text-align: center;"><h1>Query-based Analysis Across Multiple Experiments<h1></div>""",
...
if st.button("Begin the Experimental Evaluation!", key="me_stButtonCenter"):
    ...
    # Load Qrels data
    st.session_state.qme_selected_qrels = load_qrel_data(
        os.path.join(qrels_dir, qrels_file)
    )
    st.session_state.qme_max_relevance = st.session_state.qme_selected_qrels[
        "relevance"
    ].max()
    ...
    for run_file in selected_runs_files:
        st.session_state.qme_selected_runs[run_file] = load_run_data(
    ...
    if queries_file:
        st.session_state.qme_selected_queries = load_query_data(
...
if (
    "qme_selected_queries" in st.session_state
    and not st.session_state.qme_selected_queries.empty
):
    if len(st.session_state.qme_selected_queries) > 500:
        with st.container():
            st.write(
            ...
            st.write(
                f"""<div style="text-align: center;">  ⚠️ Note: Too many available queries (<span style="color:red;">{len(st.session_state.qme_selected_queries)}</span>).
            ...
            # Main content logic
            st.write(
                f"""<div style="text-align: center;"> A total of <span style="color:red;">{st.session_state.qme_random_size}</span> random queries have been
            selected based on a random state equal to <span style="color:red;">{st.session_state.qme_random_state}</span> and will be used for the upcoming analyses.</div>""",
                unsafe_allow_html=True,
            )

            st.session_state.qme_selected_queries_random = (
                st.session_state.qme_selected_queries.sample(
                    n=st.session_state.qme_random_size,
                    random_state=st.session_state.qme_random_state,
                )
            )

            st.write(
                f"""<div style="text-align: center;"> Number of randomly selected queries that would be used for analysis: <span style="color:red;"
                >{len(st.session_state.qme_selected_queries_random)}</span></div>""",
                unsafe_allow_html=True,
            )

            query_ids = np.sort(
                st.session_state.qme_selected_queries_random.query_id.values
            )
...
with st.container():
    st.markdown(
        """<h3>Retrieval Performance - <span style="color:red;">Relevance Judgments per Query</span></h3>""",
        unsafe_allow_html=True,
    )
    ...
    if "qme_selected_runs" not in st.session_state:
        ...
        st.write(
            """<h5><span style="color:red;">Query Judgement Analysis Results</span></h5>""",
            unsafe_allow_html=True,
        )
                    ...
                    with st.expander("See Analysis"):
                        if section_title == "Queries sorted based on Rel. Judgements":
                            ...
                            for label in analysis_results["query_difficulty"]:
                                ...
                                with col2in:
                                    st.write(
                                        f"**Min number of <span style='color:{color};'>{label_name}</span> Judged Documents:** {difficulty['min']:.2f}",
                                        unsafe_allow_html=True,
                                    )
                                    st.write(
                                        f"**Max number of  <span style='color:{color};'>{label_name}</span> Judged Documents:** {difficulty['max']:.2f}",
                                        unsafe_allow_html=True,
                                    )
                    ...
                    with st.expander("See Analysis"):
                        if section_title == "Relevance Labels - Statistics":
                            for label in analysis_results:
                                if label.endswith("_stats") and not label.startswith(
                                    "overall"
                                ):
                                    stats = analysis_results[label]
                                    color = (
                                        "red"
                                        if label == "Relevance_Label_0_stats"
                                        else "blue"
                                    )
                                    label_name = (
                                        "Irrelevant"
                                        if label == "Relevance_Label_0_stats"
                                        else label.split("_stats")[0]
                                    )
                                    stat_lines = []
                                    for stat in ["mean", "median", "std", "min", "max"]:
                                        if stat in stats:
                                            ...
                                            stat_lines.append(
                                                f"{stat.capitalize()}: <span style='color:red;'>{formatted_value}</span>"
                                            )
                                    stats_display = ", ".join(stat_lines)
                                    st.markdown(
                                        f"<span style='color:{color};'>{label_name}</span>: {stats_display}",
                                        unsafe_allow_html=True,
                                    )

                        elif section_title == "Overall Statistics":
                            total_judgements = sum(
                                analysis_results["overall_stats"][label]
                            ...
                            for label in analysis_results["overall_stats"]:
                                if not label.endswith("_percentage"):
                                    ...
                                    percentage = analysis_results["overall_stats"][
                                        f"{label}_percentage"
                                    ]
                                    st.markdown(
                                        f"<span style='color:{color}'>{label_name}</span>: **{count}** ({percentage:.2f}% of total)",
                                        unsafe_allow_html=True,
                                    )
                            st.write(
                                f"Total judgements:  **{total_judgements}**",
                                unsafe_allow_html=True,
                            )
...
with st.container():
    st.markdown(
        """<h3>Retrieval Performance - <span style="color:red;">Experimental Evaluation per Query</span></h3>""",
        unsafe_allow_html=True,
    )
    ...
    with st.expander("See Analysis Details and Interpretations"):
        st.subheader("Overview")
        st.write(
            "This analysis presents the retrieval performance achieved by a set of selected queries."
        )
        cola, colb = st.columns(2)
        with cola:
            st.subheader("How it works")
            st.write(
                "- The analysis leverages the ir_measures library to estimate the retrieval performance."
            )
            st.write(
                "- The graphs present the retrieval performance for all the selected measures across all experiments."
            )
        with colb:
            st.subheader("How to use")
            st.write(
                "- This analysis can be used to identify queries that have been significantly improved/downgraded across runs."
            )
            st.write(
                "- In case a initial retrieval and a re-ranking are evaluated, the graph showcases the queries that have been improved/downgraded after the re-ranking."
            )
    ...
    if "qme_selected_qrels" not in st.session_state:
        ...
        if st.session_state.qme_max_relevance >= 2:
            ...
            if (
                st.session_state.qme_relevance_threshold
                != st.session_state.qme_prev_relevance_threshold
            ):
                st.session_state.qme_prev_relevance_threshold = (
                    st.session_state.qme_relevance_threshold
                )
        ...
        with col1:
            ...
            if "qme_selected_measures" not in st.session_state:
                st.session_state.qme_selected_measures = custom_user[
                    0:4
                ]  # Default selected measures

            selected_measures = st.multiselect(
                "Select additional measures:", custom_user, default=custom_user[1:3]
            )
        ...
        if len(st.session_state.qme_selected_measures) >= 1:
            results = per_query_evaluation(
                st.session_state.qme_selected_qrels,
                st.session_state.qme_selected_runs,
                st.session_state.qme_selected_measures,
                st.session_state.qme_relevance_threshold,
            ...
            if len(st.session_state.qme_selected_runs) > 1:
                ...
                st.write(
                    """<h5><span style="color:red;">Information Related to the Analysis</span></h5>""",
                    unsafe_allow_html=True,
                )
                ...
                for i in range(num_rows):
                    ...
                    if i * 2 < len(measures):
                        ...
                        with col1:
                            st.write(
                                f"<h5>Result Analysis based on {measure}</h5>",
                                unsafe_allow_html=True,
                            )
                            with st.expander("See Analysis"):

                                same_performance = analysis_results[measure][
                                    "same_performance"
                                ]
                                if same_performance:
                                    st.write(
                                        f"**Queries with equal performance across experiments:** {', '.join(map(str, same_performance))}"
                                    )
                                    st.write("**Possible indications:**")
                                    st.write("- Queries with similar topics")
                                    st.write(
                                        "- Potential ceiling or floor effects. Indication of easy or hard queries w.r.t. the collection."
                                    )
                                else:
                                    st.write(
                                        "**No queries with consistent performance.**"
                                    )
                                    st.write(
                                        " - Suggests significant variability across experiments."
                                    )
                                ...
                                if large_gaps:
                                    st.write(
                                        f"""**Queries with large performance gaps (greater than 1.5xIQR = <span style="color:red;">{threshold:.3f})**</span>:""",
                                        unsafe_allow_html=True,
                                    )
                                    for query_id, min_val, max_val, _ in large_gaps:
                                        st.write(
                                            f"  Query {query_id}: min = {min_val:.3f}, max = {max_val:.3f}, gap = {max_val - min_val:.3f}"
                                        )
                                    st.write(
                                        """<span style="color:red;">Possible indications:</span>""",
                                        unsafe_allow_html=True,
                                    )
                                    st.write("- Sensitivity to experimental conditions")
                                    st.write(
                                        "- Areas for focused improvement, e.g. combination of the evaluated retrieval systems."
                                    )
                                else:
                                    st.write(
                                        "**No queries with large performance gaps.**"
                                    )
                                    st.write(
                                        "- Suggests consistent performance across experiments."
                                    )
            ...
            st.warning(
                "Please select at least one measure to begin your evaluation.", icon="⚠"
            )
            st.divider()
...
with st.container():
    st.markdown(
        """<h3>Retrieval Performance - <span style="color:red;">Experimental Evaluation</span> Vs <span style="color:red;">Baseline</span></h3>""",
        unsafe_allow_html=True,
    )
    _, _, custom_user, default_measures, _, _ = return_available_measures()
    ...
    with st.expander("See Analysis Details and Interpretations"):
        st.subheader("Overview")
        st.write(
            "This analysis presents the retrieval performance achieved by a set of selected queries compared to a baseline experiment."
        )
        cola, colb = st.columns(2)
        with cola:
            st.subheader("How it works")
        ...
        with colb:
            ...
            st.write(
                "- In case a initial retrieval is selected as baseline, the graph showcases the queries that have been improved/downgraded after the re-ranking."
            )

    if (
        "qme_selected_runs" not in st.session_state
        or len(st.session_state.qme_selected_runs) < 2
    ):
        st.warning(
        ...
        if len(st.session_state.qme_selected_runs) > 1:
            ...
            for run, run_analysis in analysis_results.items():
                st.write(
                    f"""<center><h5>{run} <span style="color:red;">vs</span> {baseline_run}</h5></center>""",
                    unsafe_allow_html=True,
                )
                ...
                for i in range(num_rows):
                    ...
                    if i * 2 < num_measures:
                        ...
                        with col1:
                            ...
                            st.write(
                                f"Improved Queries: {len(analysis['improved_queries'])} ({analysis['pct_improved']:.2f}%)"
                            )
                            st.write(
                                f"Degraded Queries: {len(analysis['degraded_queries'])} ({analysis['pct_degraded']:.2f}%)"
                            )
                            st.write(
                                f"Unchanged Queries: {len(analysis['unchanged_queries'])} ({analysis['pct_unchanged']:.2f}%)"
                            )
                            with st.expander("See detailed analysis"):
                                st.write(
                                    f"Average Difference: {analysis['avg_diff']:.3f}"
                                )
                                st.write(
                                    f"Median Difference: {analysis['median_diff']:.3f}"
                                )
                                st.write(
                                    f"Standard Deviation of Difference: {analysis['std_diff']:.3f}"
                                )
                                st.write("----")
                                # Additional insights
                                insights = []
                                if analysis["pct_improved"] > analysis["pct_degraded"]:
                                    ...
                                    analysis["pct_improved"] < analysis["pct_degraded"]
                                ):
                                    insights.append(
                                    ...
                                    insights.append(
                                        f"""This run shows no significant overall difference from the baseline for <span style="color:red;">{measure}</span>."""
                                    )

                                if analysis["avg_diff"] > 0:
                                    insights.append(
                                        f"""The average difference is positive (<span style="color:red;">{analysis['avg_diff']:.3f}</span>), indicating an overall improvement."""
                                    )
                                elif analysis["avg_diff"] < 0:
                                    insights.append(
                                        f"""The average difference is negative (<span style="color:red;">{analysis['avg_diff']:.3f}</span>), indicating an overall degradation."""
                                    )

                                if abs(analysis["median_diff"]) > abs(
                                    analysis["avg_diff"]
                                ):
                                    insights.append(
                                        "The median difference is larger than the average, suggesting some extreme values are influencing the results."
                                        ""
                                    )

                                if analysis["std_diff"] > abs(analysis["avg_diff"]):
                                    insights.append(
                                        """High variability in differences across queries. Some queries may have significantly larger improvements or degradations than others."""
                                    )
                                ...
                                st.write(
                                    f"- Improved Queries: {', '.join(map(str, analysis['improved_queries']))}"
                                )
                                st.write(
                                    f"- Degraded Queries: {', '.join(map(str, analysis['degraded_queries']))}"
                                )
                                st.write(
                                    f"- Unchanged Queries: {', '.join(map(str, analysis['unchanged_queries']))}"
                                )
...
with st.container():
    st.markdown(
        """<h3>Retrieval Performance - <span style="color:red;">Experimental Evaluation</span> Vs <span style="color:red;">Median/Threshold</span></h3>""",
        unsafe_allow_html=True,
    )
    _, _, custom_user, default_measures, _, _ = return_available_measures()
    ...
    with st.expander("See Analysis Details and Interpretations"):
        st.subheader("Overview")
        st.write(
            "This analysis presents the retrieval performance achieved by a set of selected queries compared to a threshold value across N evaluated experiments. This value can be either the "
        ...
        with colb:
            st.subheader("How to use")
            st.write(
                "- This analysis can be used to identify queries that have been significantly improved/downgraded across runs compared to the threshold."
            )
            st.write(
                "- In case the threshold is the median performance for each query, then one can observe an experiment for which a query performance significantly improves/downgrades compared "
    ...
    if (
        ...
        st.write(
            """If value is 0.0 (default), it compares the per query retrieval performance of the N experiment to a threshold value equal to the median performance of the selected measures, "
                 "computed based on N-1 experiments.
                 Otherwise, you can select any threshold [0.0,1.0]"""
        )

        st.session_state.qme_comparison_thresh = st.number_input(
            "Select a performance threshold (will be the same across queries):",
            min_value=0.0,
            value=0.0,
            max_value=1.0,
            step=0.05,
        ...
        results = per_query_evaluation(
            ...
            st.session_state.qme_selected_cutoff,
            None,
            st.session_state.qme_comparison_thresh,
        )
        ...
        if st.session_state.qme_comparison_thresh == 0.0:
            ...
            for run, run_analysis in analysis_results.items():
                st.write(
                    f"""<center><h5>{run} <span style="color:red;">vs</span> Median Threshold</h5></center>""",
                    unsafe_allow_html=True,
                )
                    ...
                    for j, col in enumerate([col1, col2]):
                        if i * 2 + j < num_measures:
                            ...
                            with col:
                                ...
                                st.write(
                                    f"""Improved Queries: <span style="color:red;">{len(analysis['improved_queries'])} ({analysis['pct_improved']:.2f}%)</span>""",
                                    unsafe_allow_html=True,
                                )
                                st.write(
                                    f"""Degraded Queries: <span style="color:red;">{len(analysis['degraded_queries'])} ({analysis['pct_degraded']:.2f}%)</span>""",
                                    unsafe_allow_html=True,
                                )
                                st.write(
                                    f"""Unchanged Queries: <span style="color:red;">{len(analysis['unchanged_queries'])} ({analysis['pct_unchanged']:.2f}%)</span>""",
                                    unsafe_allow_html=True,
                                )
                                with st.expander("See detailed analysis"):
                                    st.write(
                                        f"""Average Difference: <span style="color:red;">{analysis['avg_diff']:.3f}</span>""",
                                        unsafe_allow_html=True,
                                    )
                                    st.write(
                                        f"""Median Difference: <span style="color:red;">{analysis['median_diff']:.3f}</span>""",
                                        unsafe_allow_html=True,
                                    )
                                    st.write(
                                        f"""Standard Deviation of Difference: <span style="color:red;">{analysis['std_diff']:.3f}</span>""",
                                        unsafe_allow_html=True,
                                    )
                                    st.write("----")
                                    # Additional insights
                                    insights = []
                                    if (
                                        analysis["pct_improved"]
                                        > analysis["pct_degraded"]
                                    ):
                                        insights.append(
                                        ...
                                        insights.append(
                                            f"""This run shows no significant overall difference from the median for <span style="color:red;">{measure}</span>."""
                                        )
                                    ...
                                    st.write("Specifically:")
                                    st.write(
                                        f"- Queries above Median: {', '.join(map(str, analysis['improved_queries']))}"
                                    )
                                    st.write(
                                        f"- Queries below Median: {', '.join(map(str, analysis['degraded_queries']))}"
                                    )
                                    st.write(
                                        f"- Queries on Median: {', '.join(map(str, analysis['unchanged_queries']))}"
                                    )
        else:
            # This analysis leverages a threshold selected by the user
            analysis_results = analyze_performance_difference_threshold(
                results, st.session_state.qme_comparison_thresh
            )
            ...
            for run, run_analysis in analysis_results.items():
                st.write(
                    f"""<center><h5>{run} <span style="color:red;">vs</span> Threshold ({st.session_state.qme_comparison_thresh:.2f})</h5></center>""",
                    unsafe_allow_html=True,
                )
                        ...
                        if i * 2 + j < num_measures:
                            ...
                            with col:
                                ...
                                with st.expander("See detailed analysis"):
                                    ...
                                    st.write(
                                        f"Relative Improvement: {analysis['relative_improvement']:.2f}%"
                                    )
                                    st.write(
                                        f"Effect Size (Cohen's d): {analysis['effect_size']:.3f}"
                                    )
                                    st.write("----")
                                    # Additional insights
                                    insights = []
                                    if (
                                        ...
                                        insights.append(
                                            f"""This run shows no significant overall difference from the threshold for <span style="color:red;">{measure}</span>."""
                                        )
                                    ...
                                    st.write("Specifically:")
                                    st.write(
                                        f"- Queries above Threshold: {', '.join(map(str, analysis['improved_queries']))}"
                                    )
                                    st.write(
                                        f"- Queries below Threshold: {', '.join(map(str, analysis['degraded_queries']))}"
                                    )
                                    st.write(
                                        f"- Queries on Threshold: {', '.join(map(str, analysis['unchanged_queries']))}"
                                    )
